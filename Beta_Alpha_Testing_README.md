# Alpha and Beta testing in TRAPP
...
1. Once TRAPP is correctly installed, then can start setting up for running the Alpha and Beta test, including creating a Config.py file from the Config_TEMPLATE.py 

1. first want to create the zipcodes csv file, in the folder zipcodes in TRAPP can find a csv file with every zipcode in the USA, th correlating centroids(given in latitude and longitude), and the population of that zipcode from teh 2010 United Census Bureas census

2. Also found within the zipcode folder of TRAPP is the CensuscityData.py, whihc can be run to create teh correct zipcode file

1. install the map using etc and use convertor to create a passenger file for less nodes, in new york had this issue so we deleted the line, the [install](https://pypi.org/project/setuptools/) them.

1. create TAZ file, by setting districts (give estimates for each city of what the district size should be : `python setup.py install`
...
1. repeat the steps 3- 8 for each city

1.  [0.0.1 release of EPOS](https://github.com/epournaras/EPOS/releases/tag/0.0.1) and unzip it. Note the path to the path in the unzipped folder.
...
1.  Download the OVA file available at [www4.in.tum.de/~gerostat/TRAPP-SEAMS19.ova](www4.in.tum.de/~gerostat/TRAPP-SEAMS19.ova) and import it in Virtualbox. Give the VM at least 2GB of RAM to run.
